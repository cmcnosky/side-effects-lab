...
| Kernel foundation (current) | "The deterministic logical clock and ordering, run-local identifiers and SQLite ledgers, guarded operation state machine, and pure authority checks are implemented and tested; no gateway, service, or scenario is implemented." | "The lab works," any scenario-validation claim, agent evaluation, or "benchmark." |
