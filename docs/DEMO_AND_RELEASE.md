...
| Kernel foundation (current) | "The deterministic logical clock, scheduler, run-local identifiers, and guarded operation state machine are implemented and tested; no service or scenario is implemented." | "The lab works," any scenario-validation claim, agent evaluation, or "benchmark." |
